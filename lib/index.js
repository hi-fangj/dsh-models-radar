...
var FRESH_CATALOG_MS = 60 * 6e4;
var FRESH_RATINGS_MS = 15 * 6e4;
...
var EFFORT_TOKENS = /* @__PURE__ */ new Set(["ultra", "max", "xhigh", "high", "medium", "low", "off"]);
function parseRatingTierId(id) {
...
function authoritativeTrendSample(eff, benchmark, point) {
...
function normalizeRatings(window, raw, fetchedAt) {
...
function createRadarDataStore(upstream, snapshots, clock) {
  ...
  const lastRatings = /* @__PURE__ */ new Map();
  const lastHash = /* @__PURE__ */ new Map();
  const commits = /* @__PURE__ */ new Map();
  const resolveDataset = async (key, windowMs, bypass, fetcher) => {
    const now = clock.now();
    ...
    const flight = fetcher().then((value) => {
      const at = clock.now();
  ...
  const assemble = async (benchmark, defaultModel, bypass) => {
    ...
    for (const point of eff.points ?? []) {
      ...
      const hourly = Array.isArray(entries) && entries.length > 0 ? entries.filter((e) => typeof e.ts === "string").map((e) => [e.ts, num(e.score) ?? 0]) : [];
    ...
    if (Array.isArray(catalogTasks)) {
      ...
      for (const task of catalogTasks) {
        ...
        if (item.repo !== void 0 || item.language !== void 0) meta[task.id] = item;
      }
    ...
    return {
      ...
      upstreamHits: flights.filter(Boolean).reduce((hits, flight) => hits + Number(flight.upstream), 0)
    };
  };
  const enqueue = async (key, task) => {
  ...
  const enqueueCommit = async (benchmark, view) => enqueue(benchmark, () => commitSnapshot(benchmark, view));
  ...
  const readRatingsSafe = async (win) => {
    try {
  ...
  const serveRatings = (ratings, throttled) => ({
    ok: true,
    fresh: true,
    throttled: throttled || void 0,
  ...
  const getRatings = async ({
    ...
    if (!bypass) {
      const cached = datasets.get(key);
      if (cached !== void 0 && now - cached.at < FRESH_RATINGS_MS) {
        return serveRatings(cached.value, true);
      }
    ...
    try {
      ...
      return {
        ok: true,
        fresh: true,
        throttled: flight.upstream ? void 0 : true,
        fetchedAt: ratings.fetchedAt,
        data: ratings
      };
    } catch (error) {
...
var UPSTREAM_RATINGS = "https://codexradar.com";
...
var codexRadarUpstream = () => ({
  async fetchDataset(kind, benchmark) {
    ...
    return kind === "catalog" ? payload.tasks : payload;
  },
...
var ratingsPath = (window) => join(dataDir(), `latest-ratings-${window}.json`);
...
`, "utf8");
  ...
  async readRatings(window) {
    try {
      return JSON.parse(await readFile(ratingsPath(window), "utf8"));
    } catch {
      return void 0;
    }
  },
...
function apply(ctx) {
  ...
  async function handleRatings(url, res) {
    ...
    const bypass = url.searchParams.get("bypass") === "1";
    const response = await store.getRatings({ window, bypass });
    respond(res, response.ok ? 200 : 502, response);
  }
