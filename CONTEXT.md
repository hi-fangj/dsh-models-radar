...
**快照（Snapshot）**:
一次成功拉取得到的完整分数数据的带时间戳副本，持久化在插件本地。是拉取失败时的兜底显示源，并长期累积成超出远端窗口的自有历史。
...
**刷新（Refresh）**
打开设置页能力 tab 时自动发起的一次数据拉取；带轻节流：同一数据窗口内重复打开不再请求。
_Avoid_: 同步、更新
...
**总览（Overview）**:
能力总览卡：频道内每个基座以其当前最强档位参与的单屏排行，行内嵌按绝对 IQ 刻度缩放的量级条与对 24h 前的趋势信号；点击一行即把该档位设为当前模型。是打开页面的第一眼视图。
_Avoid_: 排行榜、leaderboard（界面用语中）
...
输入框下方针对当前 session 下一次请求所选模型档位的紧凑 DeepSWE 读数：当前 IQ、24h 变化与 48h 微型趋势。精确档位不存在时以 `≈` 标识同基座最强档位；基座不存在时不显示。
